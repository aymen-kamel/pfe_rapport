...
L’originalité de SmartPlayAI ne réside pas uniquement dans l’utilisation de modèles d’intelligence artificielle performants, mais dans leur intégration au sein d’un pipeline unifié capable de transformer automatiquement une vidéo de match en informations tactiques et statistiques exploitables.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{3.5cm} X C{3.5cm}}
    \rowcolor{tablehead}
    \textcolor{white}{\textbf{Composant}} &
    \textcolor{white}{\textbf{Rôle fonctionnel}} &
    \textcolor{white}{\textbf{Technologie retenue}} \\
    \rowcolor{rowalt}
    ...
    \rowcolor{rowalt}
    Visualisation et Dashboard & Afficher statistiques et heatmaps & React \\
  \end{tabularx}
...
\begin{table}[H]
  \centering
  \renewcommand{\arraystretch}{1.3}
  \begin{tabularx}{\textwidth}{X C{3cm} C{3cm} C{3.5cm}}
    \rowcolor{tablehead}
    \textcolor{white}{\textbf{Ligne reconstruite}} &
    \textcolor{white}{\textbf{Points associés}} &
    \textcolor{white}{\textbf{Dimension réelle}} &
    \textcolor{white}{\textbf{Rôle dans le système}} \\
    \rowcolor{rowalt}
    Ligne de fond basse & k1 – k2 & 10 m & Axe horizontal de référence \\
    Ligne de fond haute & k11 – k12 & 10 m & Axe horizontal de référence \\
    \rowcolor{rowalt}
    Ligne de côté gauche & k1 – k11 & 20 m & Axe vertical de référence \\
    Ligne de côté droit & k2 – k12 & 20 m & Axe vertical de référence \\
    \rowcolor{rowalt}
    Ligne de service basse & k3 – k5 & 10 m & Délimitation zones de service \\
    Ligne de service haute & k8 – k10 & 10 m & Délimitation zones de service \\
    \rowcolor{rowalt}
    Filet & k6 – k7 & 10 m & Origine du repère vertical \\
    Ligne centrale & k4 – k9 & 14 m & Séparation des couloirs de service \\
  \end{tabularx}
...
YOLOv8-Pose traite le terrain comme un objet unique auquel sont associés douze points de référence, à l'image d'un squelette dans les applications de reconnaissance de posture. Le réseau prédit simultanément les coordonnées des douze points directement en pixels, à chaque frame, ce qui permet de recalculer la transformation géométrique   et d'adapter en permanence la projection spatiale aux variations de point de vue de la caméra.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{4cm} C{3cm} X}
    ...
    \rowcolor{rowalt}
    Résolution d'entrée & 640 × 640 px & Compromis optimal précision / vitesse \\
    Sortie brute & 12 points (x, y) px & Coordonnées directement exploitables \\
    \rowcolor{rowalt}
    Seuil de confiance & 0.5 & Élimine les détections ambiguës \\
    Seuil IoU (NMS) & 0.7 & Supprime les détections redondantes \\
    \rowcolor{rowalt}
    Taille des lots & 8 frames & Exploite le parallélisme GPU \\
    Fréquence de détection & À chaque frame & Recalcul de H   \\
  \end{tabularx}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{4cm} X X}
    ...
    \textcolor{white}{\textbf{Mode statique}} &
    \textcolor{white}{\textbf{Mode dynamique}} \\
    \rowcolor{rowalt}
    Type de caméra & Fixe (trépied, perche) & Mobile (drone, caméra animée) \\
    Intervention humaine & Une fois avant l'analyse & Aucune \\
    \rowcolor{rowalt}
    Fréquence de détection & Une seule fois (mis en cache) & À chaque image \\
    Stabilité inter-images & Parfaite (zéro variation) & Dépend de la qualité de détection \\
    \rowcolor{rowalt}
    Charge de traitement & Nulle après initialisation & Constante tout au long de l'analyse \\
  \end{tabularx}
...
La transformation géométrique est recalculée immédiatement après chaque détection à partir des nouveaux points localisés. Cette recalcul   est la clé de la robustesse du mode dynamique : toute variation de l'angle ou de l'altitude de la caméra est absorbée dès la frame suivante, sans accumulation d'erreur.
...
\begin{table}[H]
  \centering
  \renewcommand{\arraystretch}{1.3}
  \begin{tabularx}{\textwidth}{X C{3.5cm} C{4cm}}
    \rowcolor{tablehead}
    \textcolor{white}{\textbf{Source de localisation}} &
    \textcolor{white}{\textbf{Précision typique}} &
    \textcolor{white}{\textbf{Contexte d'utilisation}} \\
    \rowcolor{rowalt}
    Annotation manuelle & ± 2 à 5 px & Mode statique, caméra fixe \\
    YOLOv8 & ± 3 à 8 px & Mode dynamique, caméra mobile \\
    \rowcolor{rowalt}
    Mode statique (après init.) & 0 (constant) & Caméra rigoureusement fixe \\
  \end{tabularx}
...
Le module de détection du terrain combine une approche statique par annotation unique et une approche dynamique basée sur YOLOv8-Pose afin d’assurer une localisation fiable du terrain dans différentes conditions de captation. L’utilisation de RANSAC et d’un mécanisme de persistance des transformations garantit la robustesse et la continuité de l’analyse, constituant ainsi une base géométrique essentielle pour les modules analytiques suivants.
...
constitue l’un des défis les plus exigeants de la vision par ordinateur appliquée au sport. En
raison de la vitesse élevée de déplacement, du flou de mouvement (motion blur) inhérent,
et de la petite taille apparente de l’objet dans le champ de vision, les approches de
détection image par image (frame-by-frame) s’avèrent insuﬀisantes.
Comme établi en section 1.7, (\textit{TrackNet V3}) a été retenu pour ce projet en raison de 
son module d'inpainting de trajectoire et de sa capacité à gérer les occlusions 
fréquentes dans le contexte du padel. Son architecture Encodeur-Décodeur exploite 
la continuité temporelle entre trames consécutives, ce qui le rend particulièrement 
adapté aux conditions de visibilité dégradée rencontrées dans les échanges rapides.
...
consécutives }de résolution $288 \times 512$ pixels , constituant une fenêtre 
temporelle glissante sur le flux vidéo. Cette séquence est transmise à 
TrackNet qui produit en sortie \textbf{distribution Gaussienne} \textbf{heatmap 2D} (voir section 1.7.1), 
dont le pixel de valeur maximale correspond à la position estimée de la balle.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{X C{4cm} C{4cm}}
    ...
    \textcolor{white}{\textbf{PCK @ 5px ($\uparrow$)}} &
    \textcolor{white}{\textbf{RMSE (pixels) ($\downarrow$)}} \\
    \rowcolor{rowalt}
    TrackNet (Brut) & 86,5\,\% & 4,82 px \\
    TrackNet + les filtres & \textbf{96,2\,\%} & \textbf{1,78 px} \\
  \end{tabularx}
...
étapes sont décrites dans les sections suivantes.
 ...
 \begin{figure}[H]
    \centering
    \maybeincludegraphics[width=0.7\textwidth]{images_chap2/pipeline_joueurs.png}
...
 
Le choix de YOLO26~\cite{yolo_v26} résulte d'une comparaison entre plusieurs
...
Les modèles larges sont trop coûteux pour un déploiement  .
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{L{4cm} C{3cm} X}
    ...
    \textcolor{white}{\textbf{Paramètre}} &
    \textcolor{white}{\textbf{Valeur}} &
    \textcolor{white}{\textbf{Objectif / Justification}} \\
    \rowcolor{rowalt}
    Seuil de confiance & 0,5 & Élimination des détections ambiguës et faux positifs \\
    Seuil IoU (NMS) & 0,7 & Suppression des détections redondantes (joueur unique) \\
    \rowcolor{rowalt}
    Résolution d'entrée & 1280 px & Lisibilité des silhouettes filmées en vue aérienne \\
    Classe cible & Personne & Restriction exclusive à la détection humaine \\
  \end{tabularx}
...
 
Une variante avancée intègre SAM2 (\textit{Segment Anything Model 2}) en
complément de YOLO26 : les boîtes englobantes détectées servent de
prompts au modèle de segmentation, qui génère des masques au niveau pixel.
Cette approche améliore significativement la précision de localisation,
notamment pour l'analyse de posture ou de gestuelle fine.

\paragraph{Limites}
Le gain en précision est réel mais s'accompagne de contraintes mesurées qui
justifient le caractère optionnel de cette extension :

\begin{itemize}
  \item \textbf{Latence :} l'ajout de SAM2 fait passer la latence par frame de
  \textasciitilde 22 ms (YOLO seul) à \textasciitilde 83 ms en version légère
  (vit-b) et \textasciitilde 250 ms en version complète (vit-h), soit une
  multiplication par 4 à 11, incompatible avec un traitement temps réel.
  \item \textbf{Débit :} le pipeline chute de \textasciitilde 45 fps à \textasciitilde 12 fps
  (vit-b) voire \textasciitilde 4 fps (vit-h) sur GPU RTX 3080, en deçà du
  seuil de 25 fps généralement requis pour une vidéo fluide.
  \item \textbf{Mémoire GPU :} la consommation passe de \textasciitilde 2 Go à
  \textasciitilde 6--8 Go avec SAM2 vit-h, excluant les configurations matérielles
  standard.
  \item \textbf{Robustesse :} en cas d'occlusion forte ou de chevauchement entre
  sujets, SAM2 peut produire des masques fragmentés ou erronés, dégradant la
  qualité de l'analyse plutôt que de l'améliorer.
\end{itemize}

Ces métriques montrent que le surcoût n'est justifié que lorsque la précision
de segmentation est un prérequis explicite de l'application.

\paragraph{Caractère optionnel}
Cette extension n'est pas activée par défaut. Elle reste réservée aux scénarios
où la précision prime sur la vitesse — analyses biomécaniques, étude de gestuelle
fine, post-traitement hors ligne — et peut être désactivée sans affecter le reste
du pipeline.
 
% ─────────────────────────────────────────────────────────────
\subsection{Perspectives d'amélioration}

La détection YOLO26 constitue un fondement solide, mais plusieurs axes d'optimisation méritent investigation :

\textbf{Robustesse en occlusion.} Lorsque deux joueurs se chevauchent, la prédiction de boîtes englobantes peut s'en trouver dégradée. Une approche multi-hypothèses ou l'intégration d'un module de déocclusion par apprentissage pourrait améliorer la stabilité du suivi.

\textbf{Variabilité des conditions d'éclairage.} Le modèle a été entraîné sur des données capturées en conditions maîtrisées. L'augmentation du jeu de données d'entraînement par des scènes extérieures en lumière naturelle renforcerait la généralisation.

\textbf{Latence de détection.} À 114 FPS, le traitement demeure acceptable, mais le déploiement sur dispositifs embarqués (caméras intelligentes, drones) exigerait des modèles allégés type YOLOv8n ou YOLO-NAS-Tiny sans dégrader significativement la précision.

\textbf{Gestion des cas limites.} Les joueurs partiellement visibles ou en mouvement rapide produisent occasionnellement des faux négatifs. Un post-traitement basé sur la continuité temporelle ou un module de prédiction de trajectoire permettrait de les récupérer.

\subsection{Synthèse}

Le module de suivi des joueurs repose sur une chaîne de traitement intégrant le prétraitement colorimétrique, 
la détection et le filtrage spatial des joueurs, l’attribution initiale des équipes, l’association optimale entre 
détections et pistes à l’aide de l’algorithme Hongrois, ainsi qu’un mécanisme de gestion des occlusions. Cette approche 
permet d’assurer un suivi fiable, cohérent et robuste des joueurs tout au long de la séquence vidéo.
...
permet une exploitation aussi bien   que lors d'analyses différées post-match.
...
\begin{table}[H]
  \centering
  \renewcommand{\arraystretch}{1.3}
  \begin{tabularx}{\textwidth}{L{5cm} X C{3.5cm}}
    \rowcolor{tablehead}
    ...
    \texttt{WS /analytics/live} & Flux de mise à jour des métriques & Rafraîchissement dynamique \\
  \end{tabularx}
...
le canal WebSocket (\texttt{WS}) assure la mise à jour   durant le
...
\begin{enumerate}
  ...
  \item \textbf{Mise à jour   (\textit{runtime update}) :} actualisation
...
\begin{table}[H]
  \centering
  \renewcommand{\arraystretch}{1.3}
  \begin{tabularx}{\textwidth}{L{4.5cm} X C{3cm}}
    \rowcolor{tablehead}
    \textcolor{white}{\textbf{Donnée}} &
    \textcolor{white}{\textbf{Type de visualisation}} &
    \textcolor{white}{\textbf{Bibliothèque}} \\
    \rowcolor{rowalt}
