...
\hline
Détection des joueurs & Localiser et délimiter chaque joueur & YOLOv8 \\
\hline
Analyse de posture & Extraire les 17 points clés du squelette & YOLOv8-Pose \\
\hline
Détection de la balle & Estimer la trajectoire spatio-temporelle & TrackNetV4 \\
\hline
...
% ============================================================
%  Section 4.2 — Suivi de la Balle par Réseau Convolutif Temporel
%  Rapport de PFE / Mémoire académique
%  Compatible Overleaf — pdfLaTeX
% ============================================================
%
% PACKAGES REQUIS (à placer dans votre préambule principal) :
%
...
\begin{figure}[H]
    \centering
    \maybeincludegraphics[width=0.9\textwidth]{images_chap2/pipelinecourt.png}
    \caption{Pipeline de traitement du module de détection du terrain.}
    \label{fig:Pipeline de traitement du module de détection du terrain.}
\end{figure}

\textbf{4.2.2.1 Dimensions officielles et repère de coordonnées}
...
\begin{figure}[H]
    ...
    \label{fig: Schéma des douze points de référence et des huit lignes du terrain de padel..}
\end{figure}
...
Note : les numéros de figures et de tableaux seront ajustés conformément à la numérotation définitive du rapport.

SmartPlayAI — Rapport Technique |
...
\begin{figure}[H]
    ...
    \label{fig:L'architechture du model Tracknet}
\end{figure}
...
\subsubsection*{ \textbf{Métriques d'Évaluation et Performances}}

L'évaluation quantitative du système de suivi de balle a été réalisée sur notre base de données de test à l'aide de deux métriques académiques clés :
...
\subsection*{3. Précision, Rappel et Score F1}
...
\begin{table}[h]
    ...
    \begin{tabular}{lcc}
        ...
        \textbf{Résolution d'entrée} & 640 px & Compromis optimal entre précision et vitesse temps réel \\
...
\end{table}
 
La résolution de 640\,px offre un compromis raisonné : les silhouettes
filmées à 15--20\,m de hauteur restent localisables de manière fiable,
sans surcoût computationnel incompatible avec un déploiement en temps
réel.
 
...
\begin{itemize}
    \item \textbf{Vitesse :} si le joueur était absent pendant plus de cinq images consécutives ($missed > 5$), la vitesse est réinitialisée à zéro. Sinon, elle est mise à jour à l’aide d’une EMA réactive ($\alpha_v = 0{,}5$) :
    \[
...
Afin d’obtenir une compréhension plus détaillée des actions des joueurs, la plateforme SmartPlay intègre un module d’estimation de pose basé sur YOLOv8-Pose. Ce modèle permet d’extraire automatiquement les points clés squelettiques des joueurs à partir des séquences vidéo et de représenter leur posture en temps réel.
...
\subsection{Applications dans SmartPlay}

Les points clés extraits sont exploités dans plusieurs modules analytiques :

\begin{table}[H]
\centering
\caption{Applications des points clés dans SmartPlay}
\begin{tabular}{|c|c|}
\hline
...
Le module YOLOv8-Pose constitue un élément essentiel de SmartPlay. Grâce à l’estimation des points clés, le système dépasse la simple détection des joueurs et fournit une analyse avancée des mouvements, des gestes techniques et des comportements tactiques durant les matchs de padel.
\section{Module d'analyse biomécanique : \texttt{DataAnalytics}}
\label{sec:data_analytics}
% ─────────────────────────────────────────────────────────────────────────────

\subsection{Vue d'ensemble et positionnement dans le pipeline}

Le module \texttt{DataAnalytics} constitue le composant central de la chaîne
d'analyse quantitative de \smartplayai. À chaque trame vidéo, il reçoit les positions
projetées des quatre joueurs et de la balle dans le référentiel métrique du terrain,
les accumule sous la forme de points de données horodatés, puis construit un
\textit{DataFrame} enrichi de métriques cinématiques --- déplacements, vitesses,
accélérations --- exploitées par l'ensemble des modules d'analyse et de visualisation
situés en aval du pipeline.

\subsection{Structure du référentiel de coordonnées}

L'ensemble du pipeline de calcul repose sur un référentiel cartésien centré sur le
milieu du filet. Les dimensions réglementaires FIP définissent les bornes du repère :

\begin{table}[H]
\centering
\caption{Dimensions réglementaires et repère de coordonnées du terrain.}
\label{tab:dimensions_repere}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}llcl@{}}
\toprule
\textbf{Constante} & \textbf{Valeur} & \textbf{Axe} & \textbf{Description} \\
\midrule
\texttt{BASE\_LINE}          & 10\,m & $X$ & Largeur du terrain \\
\texttt{SIDE\_LINE}          & 20\,m & $Y$ & Longueur totale \\
\texttt{SERVICE\_SIDE\_LINE} &  3\,m & $Y$ & Distance ligne de service depuis le fond de court \\
\texttt{NET\_SIDE\_LINE}     & 10\,m & $Y$ & Distance filet -- fond de court \\
\bottomrule
\end{tabular}
\end{table}

L'origine $(0,\,0)$ est placée au centre du filet (milieu $k_6$--$k_7$). L'axe~$X$
s'étend de $-5$\,m à $+5$\,m~; l'axe~$Y$ de $-10$\,m (demi-terrain supérieur) à
$+10$\,m (demi-terrain inférieur).

\subsection{Collecte des positions : projection homographique et conversion métrique}

\subsubsection{Projection homographique (\texttt{projected\_court.py})}

À chaque trame, la position du pied de chaque joueur --- exprimée en pixels dans
la vidéo originale --- est projetée dans le plan du terrain par la matrice
d'homographie $H$ calculée en amont :

\begin{equation}
  \tilde{\mathbf{p}}' = H\,\tilde{\mathbf{p}}, \qquad
  \tilde{\mathbf{p}} = \begin{pmatrix} p_x \\ p_y \\ 1 \end{pmatrix},
  \quad
  \mathbf{p}' = \frac{1}{{\tilde{p}'_3}}\begin{pmatrix} \tilde{p}'_1 \\ \tilde{p}'_2 \end{pmatrix}
  \label{eq:proj_hom}
\end{equation}

\subsubsection{Conversion pixels $\to$ mètres}

La fonction \texttt{shift\_point\_origin} soustrait l'origine (centre du filet) puis
applique le facteur d'échelle :

\begin{equation}
  x_m = \Delta p_x \cdot \frac{B_{\text{LINE}}}{W_{\text{court\_px}}},
  \qquad
  y_m = \Delta p_y \cdot \frac{S_{\text{LINE}}}{H_{\text{court\_px}}}
  \label{eq:px_to_m}
\end{equation}

où $B_{\text{LINE}} = 10$\,m, $S_{\text{LINE}} = 20$\,m, et
$W_{\text{court\_px}}$, $H_{\text{court\_px}}$ désignent les dimensions en pixels
de la mini-carte du terrain.

\subsubsection{Enregistrement par trame}

Chaque trame produit un objet \texttt{DataPoint} composé des champs suivants :
\begin{itemize}
  \item \texttt{frame} : index entier de la trame ;
  \item \texttt{players\_position} : liste de quatre instances
        \texttt{PlayerPosition(id,\,(x\_m,\,y\_m))} ;
  \item \texttt{ball\_position} : instance \texttt{BallPosition((x\_m,\,y\_m))}.
\end{itemize}

\subsection{Construction du DataFrame}

\subsubsection{Structure brute : 11 colonnes de base}

La méthode \texttt{into\_dict()} produit un dictionnaire de 11 colonnes avant
tout enrichissement cinématique :

\begin{table}[H]
\centering
\caption{Colonnes de base du DataFrame brut.}
\label{tab:df_brut}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}llll@{}}
\toprule
...
\midrule
\texttt{frame}        & \texttt{int}   & ---    & Index de trame ($0, 1, 2, \ldots$) \\
\texttt{player\{i\}\_x} & \texttt{float} & mètre  & Position $X$ du joueur $i$ \\
\texttt{player\{i\}\_y} & \texttt{float} & mètre  & Position $Y$ du joueur $i$ \\
\texttt{ball\_x}      & \texttt{float} & mètre  & Position $X$ de la balle \\
\texttt{ball\_y}      & \texttt{float} & mètre  & Position $Y$ de la balle \\
\bottomrule
\end{tabular}
\end{table}

Les valeurs manquantes, correspondant à un joueur non détecté, sont enregistrées
comme \texttt{None}, puis converties en \texttt{NaN} via
\texttt{pd.to\_numeric(\ldots, errors=\textquotesingle coerce\textquotesingle)}.

\subsubsection{Colonne \texttt{time}}
...
où $n$ est l'index de trame et $\text{fps} \in \{25, 30\}$ images par seconde.

\subsection{Pipeline de calcul cinématique}

\subsubsection{Les quatre intervalles temporels}

Afin d'obtenir des estimations de vitesse à différentes granularités temporelles,
le calcul est réalisé simultanément sur quatre intervalles $k \in \{1,2,3,4\}$ :

\begin{equation}
  \Delta t_k[n] = t[n] - t[n-k] = \frac{k}{\text{fps}}
  \label{eq:delta_t}
\end{equation}
...
\begin{tabular}{@{}ccll@{}}
\toprule
\textbf{$k$} & \textbf{$\Delta t_k$ à 30\,fps} & \textbf{Lissage} & \textbf{Usage recommandé} \\
\midrule
...
4 & $\approx 133$\,ms & Fort     & \textbf{Recommandé} --- lissage optimal pour l'analyse de performance \\
\bottomrule
\end{tabular}
\end{table}

Le choix $k=4$ est justifié par le fait qu'une fenêtre de 133\,ms correspond au seuil
minimal du temps de réaction humain. Cette valeur lisse le bruit de détection
($\pm 0{,}1$\,m entraîne une erreur de $\pm 3$\,m/s à $k=1$) sans introduire de
retard perceptible dans la détection des changements de direction réels.

\subsubsection{Déplacements $\Delta x_k$, $\Delta y_k$}

\begin{equation}
  \Delta x_k[n] = x[n] - x[n-k], \qquad
  \Delta y_k[n] = y[n] - y[n-k]
  \label{eq:deplacement}
\end{equation}

\textbf{Unité :} mètre. Plage typique en padel : $[-3{,}0\;\text{m},\;+3{,}0\;\text{m}]$
pour $k=4$ à 30\,fps.

\subsubsection{Vitesse par composante}

\begin{equation}
  V_{x,k}[n] = \frac{\Delta x_k[n]}{\Delta t_k} = \frac{x[n]-x[n-k]}{k/\text{fps}},
  \qquad
...
\subsubsection{Vitesse normée (norme euclidienne)}

\begin{equation}
  V_{\text{norm},k}[n] = \sqrt{V_{x,k}[n]^2 + V_{y,k}[n]^2}
  \label{eq:vnorm}
\end{equation}

\begin{table}[H]
\centering
\caption{Plages de vitesse caractéristiques observées en padel.}
\label{tab:vitesse_padel}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}lc@{}}
\toprule
\textbf{Situation locomotrice} & \textbf{$V_{\text{norm}}$ (m/s)} \\
\midrule
...
\subsubsection{Accélération par composante}
...
\subsubsection{Accélération normée}

\begin{equation}
  A_{\text{norm},k}[n] = \sqrt{A_{x,k}[n]^2 + A_{y,k}[n]^2}
  \label{eq:anorm}
\end{equation}

\begin{table}[H]
\centering
\caption{Plages d'accélération caractéristiques observées en padel.}
\label{tab:acceleration_padel}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}lc@{}}
\toprule
\textbf{Événement biomécanique} & \textbf{$A_{\text{norm}}$ (m/s²)} \\
\midrule
Course en régime stabilisé           & $0{,}0$ -- $2{,}0$ \\
Changement de direction              & $5{,}0$ -- $12{,}0$ \\
Départ de sprint                     & $8{,}0$ -- $15{,}0$ \\
Phase de frappe (bras, via points-clés) & $> 20{,}0$ \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Distance inter-trame}

La distance inter-trame est calculée en utilisant systématiquement $k=1$
(trames consécutives) :
...
\textbf{Unité :} mètre. La distance totale parcourue sur une séquence s'obtient par
\texttt{df[``player\{i\}\_distance''].sum()}, et se situe typiquement entre 50 et 200\,m
pour un échange complet.

\subsection{Structure complète du DataFrame enrichi}

Le DataFrame final regroupe les familles de colonnes suivantes :

\begin{description}[leftmargin=2.5cm, style=nextline]
  \item[\textbf{11 colonnes fixes}]
    \texttt{frame}, \texttt{time}, \texttt{player\{1-4\}\_\{x,y\}},
    \texttt{ball\_\{x,y\}}.

  \item[\textbf{4 colonnes temporelles}]
    \texttt{delta\_time\{1,2,3,4\}} --- intervalles $\Delta t_k$ en secondes.

  \item[\textbf{164 colonnes cinématiques}]
    Pour chaque joueur $i \in \{1,2,3,4\}$ et chaque intervalle $k \in \{1,2,3,4\}$ :
    $\Delta x_k$, $\Delta y_k$, $V_{x,k}$, $V_{y,k}$, $\Delta V_{x,k}$,
    $\Delta V_{y,k}$, $A_{x,k}$, $A_{y,k}$, $V_{\text{norm},k}$,
    $A_{\text{norm},k}$ (10 colonnes $\times$ 4 intervalles $\times$ 4 joueurs = 160)
    + \texttt{player\{i\}\_distance} ($\times\,4 = 4$).
\end{description}

\begin{equation}
  \text{Total colonnes} = 11 + 4 + 4 \times (4 \times 10 + 1) = \mathbf{179}
  \label{eq:total_cols}
\end{equation}

\subsection{Module \texttt{ShotCounter} --- Détection et classification des coups}

\subsubsection{Paramètres d'instanciation}

\begin{table}[H]
\centering
\caption{Paramètres de configuration du module \texttt{ShotCounter}.}
\label{tab:shotcounter_params}
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{@{}lllp{6cm}@{}}
\toprule
\textbf{Paramètre} & \textbf{Valeur} & \textbf{Unité} & \textbf{Interprétation physique} \\
\midrule
\texttt{proximity\_px}             & 180   & px      & $\approx 1{,}5$\,m en vue drone (résolution 1080p) \\
\texttt{cooldown\_frames}          &  25   & trames  & $\approx 0{,}83$\,s à 30\,fps \\
\texttt{direction\_cos\_threshold} & $0{,}0$ & ---   & Changement d'orientation supérieur à $90°$ \\
\texttt{speed\_change\_threshold}  & $1{,}6$ & ---   & Ratio vitesse post-impact / pré-impact \\
\texttt{min\_ball\_speed\_px}      & $1{,}5$ & px/trame & Filtrage des micro-tremblements de détection \\
\texttt{global\_cooldown\_frames}  &  12   & trames  & $\approx 0{,}4$\,s d'intervalle minimal entre deux coups \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Logique de détection en trois phases}

\paragraph{Phase 1 --- Calcul des vitesses pré/post impact.}
Sur une fenêtre glissante de $W = 5$ trames de part et d'autre du moment analysé :

\begin{equation}
  \overline{V}_x^{\,\text{pre}} = \frac{1}{W-1} \sum_{i=t-W}^{t-2} \bigl(x[i+1] - x[i]\bigr)
  \label{eq:pre_vel}
\end{equation}

\paragraph{Phase 2 --- Critères de détection.}
Un coup est détecté si au moins l'un des deux critères suivants est satisfait :

\begin{equation}
  \cos\theta = \frac{\overrightarrow{V}_{\text{pre}} \cdot
                     \overrightarrow{V}_{\text{post}}}
                    {\|\overrightarrow{V}_{\text{pre}}\|\,
                     \|\overrightarrow{V}_{\text{post}}\|}
  \label{eq:cos_theta}
\end{equation}

\begin{itemize}
  \item \textbf{Changement de direction :} $\cos\theta < 0$ (angle d'inversion $> 90°$) ;
  \item \textbf{Accélération brutale :}
    $\|\overrightarrow{V}_{\text{post}}\| / \|\overrightarrow{V}_{\text{pre}}\| > 1{,}6$.
\end{itemize}

\paragraph{Phase 3 --- Attribution au joueur le plus proche.}
La distance effective balle--joueur combine la proximité au centre de la boîte
englobante et la proximité au poignet (lorsque les points-clés articulaires sont
disponibles) :

\begin{equation}
  d_{\text{eff}} = \min\!\bigl(d_{\text{centre}},\; d_{\text{poignet}}\bigr)
  \label{eq:deff}
\end{equation}

Le seuil de proximité est adaptatif : 150\,px en situation standard, porté à
200--250\,px lors des phases de smash (vitesse balle élevée et $\cos\theta < 0{,}5$).

\subsubsection{Classification des types de coups}

La méthode \texttt{classify\_shot()} délègue la classification au sous-module
\texttt{PadelShotClassifier}, qui identifie le type de coup parmi les six catégories
suivantes : \textit{Smash}, \textit{Bandeja}, \textit{Forehand}, \textit{Backhand},
\textit{Volley} et \textit{Service}. L'extraction de caractéristiques analyse une
fenêtre temporelle de $\pm 10$ trames (soit 700\,ms à 30\,fps) autour du point
d'impact, en intégrant 21 distances balle--poignet, la hauteur relative de la balle
par rapport à la tête et les angles articulaires épaule--coude--poignet.

\subsection{Automate de scoring (\texttt{PadelScorer})}

Le module \texttt{PadelScorer} implémente une machine à états finis (FSM) modélisant
les règles de scoring du padel :

\begin{figure}[H]
\centering
\begin{tikzpicture}[
  state/.style={draw, rounded corners=4pt, fill=lightgray,
  ...
  \node[state] (idle)   {IDLE};
  \node[state, right=3cm of idle]  (play)  {IN\_PLAY};
  \node[state, right=3cm of play]  (bounce){BOUNCED};
  \node[state, below=1.5cm of play](ended) {POINT\\ENDED};

  \draw[arrow] (idle)   -- node[above, label]{hit}        (play);
  \draw[arrow] (play)   -- node[above, label]{bounce(1)}  (bounce);
  \draw[arrow] (bounce) -- node[right, label]{hit}         (play);
  \draw[arrow] (bounce.south) -- node[right, label]{\footnotesize bounce(2, même côté)} (ended);
  \draw[arrow] (ended.west)   -| node[below, label]{reset} (idle);
\end{tikzpicture}
\caption{Machine à états finis du module \texttt{PadelScorer}.}
\label{fig:fsm_scorer}
\end{figure}

La règle de point est conforme au règlement officiel du padel : deux rebonds
consécutifs du même côté du filet concèdent le point à l'équipe adverse. Le score
est affiché selon la progression tennis classique : $0 \to 15 \to 30 \to 40 \to
\text{Jeu}$.

\subsection{Vue d'ensemble du flux de données}

\begin{table}[H]
\centering
\caption{Flux entrée $\to$ traitement $\to$ sortie du module \texttt{DataAnalytics}.}
\label{tab:flux_analytics}
\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{@{}lXl@{}}
\toprule
\textbf{Étape} & \textbf{Description} & \textbf{Format} \\
\midrule
Entrée        & Positions des joueurs et de la balle (espace pixel vidéo) & \texttt{(px, py)} \\
Projection    & Transformation homographique et conversion métrique        & \texttt{(x\_m, y\_m)} \\
Accumulation  & Construction d'un objet \texttt{DataPoint} par trame ($N$ trames) & Liste de dictionnaires \\
...
Export CSV    & \texttt{data\_new.csv} ($\approx 3{,}2$\,Mo pour un match complet) & \texttt{.csv} \\
\bottomrule
\end{tabularx}
\end{table}

\begin{synthesebox}[Synthèse --- Module DataAnalytics]
Le module \texttt{DataAnalytics} transforme les coordonnées brutes en pixels en un
DataFrame de 179 colonnes structuré autour de quatre niveaux de lissage cinématique.
La stratégie multi-intervalle $k \in \{1,2,3,4\}$ offre un compromis entre réactivité
et lissage~; la valeur $k=4$ (fenêtre de 133\,ms) est recommandée pour l'analyse des
performances. Le sous-module \texttt{ShotCounter} enrichit ce flux par la détection
événementielle des coups et leur classification biomécanique en six catégories, tandis
que \texttt{PadelScorer} assure le suivi du score via une FSM conforme aux règles
officielles du padel.
\end{synthesebox}
